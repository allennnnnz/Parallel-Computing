#include <iostream>
#include <cstdlib>
#include <string>
#include <utility>
#include <cmath>
#include <chrono>
#include <iomanip>

#include <openacc.h>

#include "mnist/mnist_reader.hpp"

using namespace std;

#define RESULT_FILE "result.txt"
#define MNIST_DATA_LOCATION "/share/testcases/lab-mnist/MNIST"
#define WEIGHT_ROOT "/share/testcases/lab-mnist/weights"

/* Model architecture:
 *  Layer1: 784 x 1024
 *  Layer2: 1024 x 10
 */
#define LAYER0 784
#define LAYER1 1024
#define LAYER2 10

/* ============================
 * GPU kernels (OpenACC)
 * ============================ */

/* LinearLayer:
 * D[n,m] = A[n,k] * B[k,m] + C[m]
 * A: (n x k)
 * B: (k x m)
 * C: (m)
 * D: (n x m)
 *
 * 假設 A,B,C 已在 device，D 在 device
 */
static inline
void LinearLayer(float *A, float *B, float *C, float *D,
                 int n, int k, int m)
{
    // 平行展開 (i,j)，內層 a 累加做 reduction
    // gang vector 給編譯器強烈並行 hint
#pragma acc parallel loop gang vector collapse(2) \
    present(A[0:n*k], B[0:k*m], C[0:m], D[0:n*m])
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {

            float sum = C[j];
#pragma acc loop seq reduction(+:sum)
            for (int a = 0; a < k; a++) {
                sum += A[i * k + a] * B[a * m + j];
            }
            D[i * m + j] = sum;
        }
    }
}

/* Sigmoid:
 * A[n,m] = sigmoid(A[n,m])
 */
static inline
void Sigmoid(float *A, int n, int m)
{
#pragma acc parallel loop gang vector collapse(2) \
    present(A[0:n*m])
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            float x = A[i * m + j];
            A[i * m + j] = 1.0f / (1.0f + expf(-x));
        }
    }
}

/* Argmax:
 * A[n,m] (float scores)
 * D[n]   (int class index)
 * row-wise argmax
 *
 * 我們在 row 維度上平行，row 內部用 seq loop。
 */
static inline
void Argmax(float *A, int *D, int n, int m)
{
#pragma acc parallel loop gang vector \
    present(A[0:n*m], D[0:n])
    for (int i = 0; i < n; i++) {
        float mx = A[i * m + 0];
        int idx  = 0;
        for (int j = 1; j < m; j++) {
            float v = A[i * m + j];
            if (v > mx) {
                mx = v;
                idx = j;
            }
        }
        D[i] = idx;
    }
}

/* 推論網路: 
 * training_images_flat: num_images x LAYER0
 * layer1_matrix:       LAYER0 x LAYER1
 * layer1_bias:         LAYER1
 * layer2_matrix:       LAYER1 x LAYER2
 * layer2_bias:         LAYER2
 * layer1_output:       num_images x LAYER1  (scratch)
 * layer2_output:       num_images x LAYER2  (scratch)
 * result:              num_images
 *
 * 這裡假設所有 buffer 都已經在 device data region 中 (present)
 */
static inline
void my_nn(float *training_images_flat, int num_images,
           float *layer1_matrix, float *layer1_bias,
           float *layer2_matrix, float *layer2_bias,
           float *layer1_output, float *layer2_output,
           int *result)
{
    // Layer 1
    LinearLayer(training_images_flat,
                layer1_matrix,
                layer1_bias,
                layer1_output,
                num_images, LAYER0, LAYER1);

    Sigmoid(layer1_output, num_images, LAYER1);

    // Layer 2
    LinearLayer(layer1_output,
                layer2_matrix,
                layer2_bias,
                layer2_output,
                num_images, LAYER1, LAYER2);

    Argmax(layer2_output, result, num_images, LAYER2);
}

/* ============================
 * Host-side helpers (unchanged)
 * ============================ */

void read_weight(float *array, string filename, int num_floats) {
    string full_filename = string(WEIGHT_ROOT) + '/' + filename;
    std::cout << "Reading file: " << full_filename << std::endl;
    ifstream file(full_filename, ios::in | ios::binary);
    if (!file) {
        std::cerr << "error reading file: " << full_filename << std::endl;
        exit(1);
    }
    file.read((char*)array, num_floats * sizeof(float));
}

void write_predict(int *result, int n, string filename) {
    std::ofstream file(filename, std::ofstream::out);
    for (int i = 0; i < n; i++) {
        file << result[i] << '\n';
    }
    file.close();
}

void print_img(float *img) {
    for (int i = 0; i < 28; i++) {
        for (int j = 0; j < 28; j++) {
            if (img[i*28+j] > 0.5) {
                std::cout << 'x';
            } else {
                std::cout << ' ';
            }
        }
        std::cout << '\n';
    }
    std::cout << std::endl;
}

bool InitCUDA(void) {
    int count = acc_get_num_devices(acc_device_nvidia);
    if(count == 0) {
        std::cerr << "There is no device.\n";
        return false;
    }
    acc_set_device_num(0, acc_device_nvidia);
    std::cout << "CUDA initialized.\n";
    return true;
}

/* ============================
 * main()
 * ============================ */

int main(int argc, char* argv[]) {
    auto initcuda_start = std::chrono::steady_clock::now();
    InitCUDA();
    auto read_start = std::chrono::steady_clock::now();

    // Load MNIST
    mnist::MNIST_dataset<std::vector, std::vector<uint8_t>, uint8_t> dataset;
    if (argc == 1) {
        dataset = mnist::read_dataset<std::vector, std::vector, uint8_t, uint8_t>(MNIST_DATA_LOCATION);
    } else {
        dataset = mnist::read_dataset<std::vector, std::vector, uint8_t, uint8_t>(argv[1]);
    }

    int num_train_images = dataset.training_images.size();
    int num_pixels       = dataset.training_images.front().size();  // 784

    // flatten images to float [0,1]
    float *training_images_flat = new float[num_train_images * num_pixels];
    float **training_images = new float*[num_train_images];
    for (int i = 0; i < num_train_images; i++) {
        training_images[i] = training_images_flat + i * num_pixels;
    }

    for (int i = 0; i < num_train_images; i++) {
        for (int j = 0; j < num_pixels; j++) {
            training_images[i][j] = (float)(dataset.training_images[i][j]) / 255.0f;
        }
    }

    // load weights
    float *layer1_matrix = new float[LAYER0 * LAYER1];
    float *layer1_bias   = new float[LAYER1];
    float *layer2_matrix = new float[LAYER1 * LAYER2];
    float *layer2_bias   = new float[LAYER2];

    read_weight(layer1_matrix, "layer1_matrix", LAYER0 * LAYER1);
    read_weight(layer1_bias,   "layer1_bias",   LAYER1);
    read_weight(layer2_matrix, "layer2_matrix", LAYER1 * LAYER2);
    read_weight(layer2_bias,   "layer2_bias",   LAYER2);

    auto read_end = std::chrono::steady_clock::now();

    // allocate inference outputs
    float *layer1_output = new float[num_train_images * LAYER1];
    float *layer2_output = new float[num_train_images * LAYER2];
    int   *result        = new int[num_train_images];

    // ============================
    // OpenACC data region (one-shot)
    // ============================
#pragma acc data copyin( training_images_flat[0:num_train_images*num_pixels], \
                          layer1_matrix[0:LAYER0*LAYER1], \
                          layer1_bias[0:LAYER1], \
                          layer2_matrix[0:LAYER1*LAYER2], \
                          layer2_bias[0:LAYER2] ) \
                 create( layer1_output[0:num_train_images*LAYER1], \
                         layer2_output[0:num_train_images*LAYER2] ) \
                 copyout( result[0:num_train_images] )
    {
        my_nn(training_images_flat, num_train_images,
              layer1_matrix, layer1_bias,
              layer2_matrix, layer2_bias,
              layer1_output, layer2_output,
              result);
    }

    auto inference_end = std::chrono::steady_clock::now();

    // accuracy check on host
    int correct = 0;
    int total = 0;
    for (int i = 0; i < num_train_images; i++) {
        if ((int)result[i] == (int)dataset.training_labels[i]) {
            correct++;
        }
        total++;
    }

    std::cout << "\nInference accuracy: "
              << (double)correct / (double)total * 100.0
              << "%\n";

    if (argc == 1)
        write_predict(result, num_train_images, RESULT_FILE);
    else
        write_predict(result, num_train_images, argv[2]);

    auto acc_end = std::chrono::steady_clock::now();

    std::cout << std::setprecision(5) << std::fixed;
    std::cout << "\n-----     STATS     -----\n";
    std::cout << "Time for initializing CUDA device:     "
              << std::chrono::duration_cast<std::chrono::milliseconds>(read_start - initcuda_start).count() << " m.s.\n";
    std::cout << "Time for reading MNIST data & weights: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(read_end - read_start).count() << " m.s.\n";
    std::cout << "Time for inferencing                 : "
              << std::chrono::duration_cast<std::chrono::milliseconds>(inference_end - read_end).count() << " m.s.\n";
    std::cout << "Time for calculating accuracy        : "
              << std::chrono::duration_cast<std::chrono::milliseconds>(acc_end - inference_end).count() << " m.s.\n";
    std::cout <<   "----- END OF STATS  -----\n";

    // cleanup host memory
    delete [] result;
    delete [] layer1_output;
    delete [] layer2_output;
    delete [] layer1_matrix;
    delete [] layer1_bias;
    delete [] layer2_matrix;
    delete [] layer2_bias;
    delete [] training_images_flat;
    delete [] training_images;

    return 0;
}